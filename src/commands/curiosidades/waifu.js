...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('waifu')
        .setDescription('Envía un GIF de una waifu aleatoria.'),
    async execute(interaction) {
        console.log(`Comando /waifu ejecutado por ${interaction.user.tag}`);
        await interaction.deferReply(); // Defer the reply immediately

        const searchQuery = 'anime waifu';
        ...
        if (gifUrl && !gifUrl.startsWith('Error')) { // Check if it's a valid URL and not an error message
            ...
            await interaction.editReply({ content: '¡Aquí tienes una waifu!', files: [gifUrl] }); // Use editReply
