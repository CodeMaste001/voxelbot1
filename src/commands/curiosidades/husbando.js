...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('husbando')
        .setDescription('Envía un GIF de un husbando aleatorio.'),
    async execute(interaction) {
        console.log(`Comando /husbando ejecutado por ${interaction.user.tag}`);
        await interaction.deferReply(); // Defer the reply immediately

        const searchQuery = 'anime husbando';
        ...
        if (gifUrl && !gifUrl.startsWith('Error')) { // Check if it's a valid URL and not an error message
            ...
            await interaction.editReply({ content: '¡Aquí tienes un husbando!', files: [gifUrl] }); // Use editReply
