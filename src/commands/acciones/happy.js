...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('happy')
        .setDescription('Envía un GIF feliz.'),
    async execute(interaction) {
        console.log(`Comando /happy ejecutado por ${interaction.user.tag}`);
        await interaction.deferReply(); // Defer the reply immediately

        const searchQuery = 'anime happy';
        ...
        if (gifUrl && !gifUrl.startsWith('Error')) { // Check if it's a valid URL and not an error message
            ...
            await interaction.editReply({ content: '¡Qué alegría!', files: [gifUrl] }); // Use editReply
