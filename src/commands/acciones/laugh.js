...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('laugh')
        .setDescription('El bot ríe.'),
    async execute(interaction) {
        console.log(`Comando /laugh ejecutado por ${interaction.user.tag}`);
        await interaction.deferReply(); // Defer the reply immediately

        const searchQuery = 'anime laugh';
        ...
        if (gifUrl && !gifUrl.startsWith('Error')) { // Check if it's a valid URL and not an error message
            ...
            await interaction.editReply({ content: '¡Jajaja!', files: [gifUrl] }); // Use editReply
