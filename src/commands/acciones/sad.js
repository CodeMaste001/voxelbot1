...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('sad')
        .setDescription('Envía un GIF triste.'),
    async execute(interaction) {
        console.log(`Comando /sad ejecutado por ${interaction.user.tag}`);
        await interaction.deferReply(); // Defer the reply immediately

        const searchQuery = 'anime sad';
        ...
        if (gifUrl && !gifUrl.startsWith('Error')) { // Check if it's a valid URL and not an error message
            ...
            await interaction.editReply({ content: '¡Ánimo!', files: [gifUrl] }); // Use editReply
