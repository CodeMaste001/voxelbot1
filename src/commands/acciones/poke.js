...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('poke')
        .setDescription('Envía un GIF de “poke” o empujón.')
        .addUserOption(option =>
            option.setName('usuario')
                .setDescription('El usuario a empujar.')
                .setRequired(true)),
    async execute(interaction) {
        console.log(`Comando /poke ejecutado por ${interaction.user.tag}`);
        await interaction.deferReply(); // Defer the reply immediately

        const user = interaction.options.getUser('usuario');
        const searchQuery = 'anime poke';
        ...
        if (gifUrl && !gifUrl.startsWith('Error')) { // Check if it's a valid URL and not an error message
            ...
            await interaction.editReply({ content: `¡${interaction.user} le ha dado un empujón a ${user}!`, files: [gifUrl] }); // Use editReply
